...
def test_multiple_writes_latest_points_at_most_recent(tmp_path):
    ...
    write_snapshot(df1, table="t1", root=tmp_path, timestamp=datetime(2025, 1, 1, tzinfo=UTC))
    write_snapshot(df2, table="t1", root=tmp_path, timestamp=datetime(2025, 1, 2, tzinfo=UTC))
    result = read_latest("t1", root=tmp_path)
